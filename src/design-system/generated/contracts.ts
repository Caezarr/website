/* This file is generated by `bun run ds:build`. Do not edit directly. */

export const designSystemManifest = {
  "$schema": "/design-system/schemas/manifest.schema.json",
  "schemaVersion": "1.0.0",
  "id": "wonka-design-system",
  "name": "Wonka Design System",
  "version": "0.3.0",
  "brandVersionId": "wonka-brand-v0.1.0",
  "status": "beta",
  "approval": {
    "status": "review_required",
    "approvedBy": null,
    "approvedAt": null
  },
  "description": "The canonical brand and interface system for Wonka products, campaigns, presentations, and agent-generated artifacts.",
  "principles": [
    {
      "id": "sovereign-clarity",
      "name": "Sovereign clarity",
      "description": "Make complex enterprise AI feel controlled, legible, and calm."
    },
    {
      "id": "editorial-precision",
      "name": "Editorial precision",
      "description": "Pair an editorial display voice with disciplined product typography."
    },
    {
      "id": "useful-motion",
      "name": "Useful motion",
      "description": "Motion explains state and hierarchy; it never competes with the task."
    },
    {
      "id": "machine-readable",
      "name": "Machine-readable by default",
      "description": "Every approved rule has a stable identifier, version, description, and structured representation."
    }
  ],
  "themes": [
    {
      "id": "light",
      "label": "Light",
      "selector": "[data-theme=\"light\"]",
      "status": "stable"
    },
    {
      "id": "dark",
      "label": "Dark",
      "selector": "[data-theme=\"dark\"]",
      "status": "stable"
    }
  ],
  "channels": [
    "product",
    "website",
    "campaign",
    "presentation"
  ],
  "sources": {
    "tokens": "packages/tokens/src/wonka.tokens.json",
    "components": "design-system/components.json",
    "assets": "design-system/assets.json",
    "rules": "design-system/rules/catalog.json",
    "exceptions": "design-system/rules/exceptions.json",
    "patterns": "design-system/patterns.json",
    "channels": "design-system/channels"
  },
  "generated": {
    "css": "packages/tokens/src/generated/tokens.css",
    "typescript": "packages/tokens/src/generated/tokens.ts",
    "tokenCatalog": "public/design-system/tokens.json",
    "componentCatalog": "public/design-system/components.json",
    "assetCatalog": "public/design-system/assets.json",
    "ruleCatalog": "public/design-system/rules.json",
    "exceptionCatalog": "public/design-system/exceptions.json",
    "patternCatalog": "public/design-system/patterns.json",
    "channelCatalog": "public/design-system/channels.json",
    "contractTypescript": "src/design-system/generated/contracts.ts",
    "manifest": "public/design-system/manifest.json",
    "agentGuide": "public/design-system/llms.txt",
    "discovery": "public/.well-known/design-system.json"
  },
  "interfaces": {
    "storybook": {
      "script": "bun run storybook",
      "buildScript": "bun run storybook:build",
      "publicUrl": "https://design-system.wonka-ai.com",
      "runtimeStoryPathTemplate": "/?path=/story/{storyId}"
    },
    "agentCli": {
      "script": "bun run ds:query --",
      "commands": [
        "manifest",
        "tokens",
        "token <id> [--theme light|dark]",
        "components",
        "component <id>",
        "assets",
        "asset <id>",
        "rules",
        "rule <id>",
        "exceptions",
        "patterns [--channel <id|alias>]",
        "pattern <id>",
        "channels",
        "channel <id|alias>",
        "policy --channel <id|alias>",
        "search <query>"
      ]
    }
  },
  "governance": {
    "lifecycle": [
      "draft",
      "beta",
      "stable",
      "deprecated",
      "retired"
    ],
    "breakingChangesRequireMajorVersion": true,
    "humanApprovalRequired": true,
    "rawValuesAllowedInConsumers": false
  }
} as const;

export const ruleCatalog = {
  "$schema": "/design-system/schemas/rule-catalog.schema.json",
  "schemaVersion": "1.0.0",
  "version": "0.3.0",
  "rules": [
    {
      "id": "rule.no-raw-color",
      "name": "No embedded hex or functional colors",
      "description": "New design-system code must reference approved tokens instead of embedding hex values or CSS color-function values.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "packages/react/src/**/*.tsx",
          "src/app/design-system/**/*.tsx",
          "src/design-system/**/*.tsx"
        ],
        "exclude": [
          "**/generated/**"
        ]
      },
      "validator": {
        "kind": "raw_color",
        "pattern": "#(?:[0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})(?![0-9a-f])|(?:rgba?|hsla?|hwb|lab|lch|oklab|oklch|color|device-cmyk)\\s*\\(",
        "flags": "i"
      },
      "rationale": "A single canonical token source prevents channel and theme drift.",
      "exceptionsAllowed": true,
      "provenance": {
        "source": "Wonka design-system production hardening",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.approved-radius",
      "name": "No oversized generic radius utilities",
      "description": "New surfaces avoid Tailwind xl, 2xl, and 3xl radius utilities in favor of the restrained Wonka radius scale.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "packages/react/src/**/*.tsx",
          "src/app/design-system/**/*.tsx",
          "src/design-system/**/*.tsx"
        ],
        "exclude": [
          "**/generated/**"
        ]
      },
      "validator": {
        "kind": "forbidden_pattern",
        "pattern": "\\brounded-(?:xl|2xl|3xl)\\b"
      },
      "rationale": "Shape consistency is one of the clearest signals that outputs belong to the same brand.",
      "exceptionsAllowed": true,
      "provenance": {
        "source": "Existing homepage design contract",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.contrast-aa",
      "name": "WCAG AA component contrast",
      "description": "Text and visible focus states meet their WCAG 2.2 AA contrast thresholds in every supported theme.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.1.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "packages/tokens/src/**/*.json"
        ],
        "exclude": []
      },
      "validator": {
        "kind": "contrast",
        "pairs": [
          {
            "id": "component.button.primary",
            "foreground": "component.button.primary.foreground",
            "background": "component.button.primary.background",
            "minimum": 4.5
          },
          {
            "id": "component.button.secondary",
            "foreground": "component.button.secondary.foreground",
            "background": "component.button.secondary.background",
            "minimum": 4.5
          },
          {
            "id": "component.badge",
            "foreground": "semantic.color.text",
            "background": "semantic.color.surface-muted",
            "minimum": 4.5
          },
          {
            "id": "component.focus-ring",
            "foreground": "component.focus-ring.color",
            "background": "semantic.color.background",
            "minimum": 3
          }
        ]
      },
      "rationale": "Brand expression must remain usable for people with low vision and in constrained display conditions.",
      "exceptionsAllowed": false,
      "provenance": {
        "source": "WCAG 2.2 AA",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.asset-rights",
      "name": "Verified asset rights",
      "description": "Only assets with verified rights may become active or ship in generated artifacts.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.1.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "design-system/assets.json"
        ],
        "exclude": []
      },
      "validator": {
        "kind": "asset_rights"
      },
      "rationale": "Agents must not turn an unverified font or logo file into an approved distribution right.",
      "exceptionsAllowed": false,
      "provenance": {
        "source": "Wonka Brand DNA governance",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.generated-no-drift",
      "name": "Generated artifacts stay in sync",
      "description": "Committed CSS, TypeScript, public catalogs, and package exports must match their canonical sources.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.1.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "design-system/**/*.json",
          "design-system/**/*.txt"
        ],
        "exclude": []
      },
      "validator": {
        "kind": "generated_drift"
      },
      "rationale": "Humans and agents must see the same version of every approved decision.",
      "exceptionsAllowed": false,
      "provenance": {
        "source": "Wonka design-system build contract",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.lifecycle-approval",
      "name": "Human approval before activation",
      "description": "A rule, channel, asset, or brand version cannot become active without named human approval and a timestamp.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "design-system/**/*.json",
          "packages/*/package.json"
        ],
        "exclude": []
      },
      "validator": {
        "kind": "lifecycle_approval"
      },
      "rationale": "Automation may prepare and validate decisions, but it cannot grant brand or publication authority.",
      "exceptionsAllowed": false,
      "provenance": {
        "source": "Wonka design-system governance",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    },
    {
      "id": "rule.agent-traceability",
      "name": "Agent output traceability",
      "description": "Agent-generated artifacts report the exact system, channel, tokens, components, and assets used.",
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "severity": "error",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "scope": {
        "include": [
          "agent-output/**/*"
        ],
        "exclude": []
      },
      "validator": {
        "kind": "manual",
        "checklist": [
          "Report designSystemVersion and brandVersionId.",
          "Report the selected channel id and theme.",
          "Report every token id, component id, and asset path used.",
          "Report unresolved review_required decisions."
        ]
      },
      "rationale": "A deterministic trace lets humans and downstream agents reproduce, review, and safely revise generated work.",
      "exceptionsAllowed": false,
      "provenance": {
        "source": "Wonka AI-native design-system contract",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      }
    }
  ]
} as const;

export const exceptionCatalog = {
  "$schema": "/design-system/schemas/rule-exceptions.schema.json",
  "schemaVersion": "1.0.0",
  "version": "0.3.0",
  "exceptions": []
} as const;

export const patternCatalog = {
  "$schema": "/design-system/schemas/pattern-catalog.schema.json",
  "schemaVersion": "1.0.0",
  "version": "0.3.0",
  "brandVersionId": "wonka-brand-v0.1.0",
  "patterns": [
    {
      "id": "pattern.system.brand-os",
      "name": "Brand OS overview",
      "description": "A system-level view that explains the canonical source, policy flow, machine interfaces, and trace contract.",
      "kind": "documentation",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/introduction.stories.tsx",
      "storyId": "getting-started-introduction--overview",
      "channelIds": [
        "channel.product",
        "channel.website",
        "channel.campaign",
        "channel.presentation"
      ],
      "requiredTokenSets": [
        "semantic.color",
        "typography",
        "radius"
      ],
      "requiredComponentIds": [
        "component.badge"
      ],
      "requiredAssetIds": [],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "system-status",
          "role": "evidence",
          "required": true,
          "instruction": "Show the exact design-system version, brand version, approval state, themes, and interface status."
        },
        {
          "id": "operating-model",
          "role": "sequence",
          "required": true,
          "instruction": "Explain the deterministic path from channel intent to policy, composition, and trace."
        },
        {
          "id": "machine-surface",
          "role": "interface",
          "required": true,
          "instruction": "Expose stable discovery and catalog endpoints without requiring rendered-page scraping."
        }
      ],
      "agent": {
        "requiredInputs": [
          "audience",
          "channel",
          "theme"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions"
        ],
        "humanReviewTriggers": [
          "Changing approval state",
          "Presenting a beta contract as stable",
          "Publishing a new brand version"
        ]
      },
      "usage": {
        "do": [
          "Use this as the entry point for human and agent onboarding.",
          "Keep system status derived from generated contracts."
        ],
        "dont": [
          "Hard-code approval or inventory counts.",
          "Imply that review-required decisions are effective."
        ]
      }
    },
    {
      "id": "pattern.governance.agent-workbench",
      "name": "Agent policy workbench",
      "description": "An interactive governance surface for selecting a channel, inspecting readiness, and reviewing the trace an agent must return.",
      "kind": "governance",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/agent-workbench.stories.tsx",
      "storyId": "governance-agent-workbench--workbench",
      "channelIds": [
        "channel.product"
      ],
      "requiredTokenSets": [
        "semantic.color",
        "typography",
        "radius",
        "motion"
      ],
      "requiredComponentIds": [
        "component.badge",
        "component.button",
        "component.surface"
      ],
      "requiredAssetIds": [],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "channel-selector",
          "role": "control",
          "required": true,
          "instruction": "Offer every canonical channel and expose the stable channel ID."
        },
        {
          "id": "policy-result",
          "role": "evidence",
          "required": true,
          "instruction": "Separate effective, candidate, and blocked decisions."
        },
        {
          "id": "trace-preview",
          "role": "interface",
          "required": true,
          "instruction": "Preview machine-readable provenance for the selected channel."
        }
      ],
      "agent": {
        "requiredInputs": [
          "channel",
          "theme",
          "artifactIntent"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions"
        ],
        "humanReviewTriggers": [
          "Any candidate or blocked dependency",
          "Any rights-verification-required asset",
          "Any requested policy exception"
        ]
      },
      "usage": {
        "do": [
          "Resolve policy before presenting a recommended composition.",
          "Keep channel selection keyboard operable."
        ],
        "dont": [
          "Merge candidate and effective decisions.",
          "Hide approval or rights blockers."
        ]
      }
    },
    {
      "id": "pattern.product.agent-workspace",
      "name": "Agent workspace",
      "description": "A task-oriented product shell that keeps agent activity, source context, approvals, and next actions legible.",
      "kind": "artifact",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/artifact-lab.stories.tsx",
      "storyId": "compositions-artifact-lab--studio",
      "channelIds": [
        "channel.product"
      ],
      "requiredTokenSets": [
        "semantic.color",
        "component",
        "typography",
        "motion",
        "radius"
      ],
      "requiredComponentIds": [
        "component.badge",
        "component.button",
        "component.surface"
      ],
      "requiredAssetIds": [],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "task-context",
          "role": "content",
          "required": true,
          "instruction": "Name the user goal, connected sources, and current agent state."
        },
        {
          "id": "activity-trace",
          "role": "evidence",
          "required": true,
          "instruction": "Show consequential tool activity and the evidence used."
        },
        {
          "id": "approval-action",
          "role": "control",
          "required": true,
          "instruction": "Make the next human decision explicit and reversible."
        }
      ],
      "agent": {
        "requiredInputs": [
          "task",
          "sourceContext",
          "approvalBoundary"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions",
          "sources",
          "actions",
          "approval"
        ],
        "humanReviewTriggers": [
          "External write or message",
          "Access-control change",
          "Irreversible action"
        ]
      },
      "usage": {
        "do": [
          "Keep the current task and system state visible together.",
          "Make pending approval distinct from progress."
        ],
        "dont": [
          "Use decorative activity that resembles real agent execution.",
          "Collapse evidence and final output into an ambiguous feed."
        ]
      }
    },
    {
      "id": "pattern.website.editorial-landing",
      "name": "Editorial landing",
      "description": "A conversion-oriented website composition pairing editorial authority with explicit product evidence.",
      "kind": "artifact",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/artifact-lab.stories.tsx",
      "storyId": "compositions-artifact-lab--studio",
      "channelIds": [
        "channel.website"
      ],
      "requiredTokenSets": [
        "semantic.color",
        "component",
        "typography",
        "motion",
        "radius"
      ],
      "requiredComponentIds": [
        "component.button",
        "component.eyebrow",
        "component.section",
        "component.surface"
      ],
      "requiredAssetIds": [
        "asset.logo.wordmark"
      ],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "positioning",
          "role": "content",
          "required": true,
          "instruction": "Lead with one differentiated outcome in plain language."
        },
        {
          "id": "product-evidence",
          "role": "evidence",
          "required": true,
          "instruction": "Pair the promise with a concrete product or governance proof."
        },
        {
          "id": "primary-action",
          "role": "control",
          "required": true,
          "instruction": "Use one explicit primary conversion action."
        }
      ],
      "agent": {
        "requiredInputs": [
          "audience",
          "positioning",
          "primaryAction"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions",
          "claims",
          "primaryAction"
        ],
        "humanReviewTriggers": [
          "Unverified claim",
          "New logo treatment",
          "New conversion or legal copy"
        ]
      },
      "usage": {
        "do": [
          "Balance editorial display type with product-grade evidence.",
          "Keep the signal rail to one intentional appearance."
        ],
        "dont": [
          "Stack interchangeable marketing cards.",
          "Use generic gradients as brand expression."
        ]
      }
    },
    {
      "id": "pattern.campaign.signal-creative",
      "name": "Signal campaign creative",
      "description": "A campaign system that preserves one brand signal, a deterministic claim layer, and adaptable safe zones.",
      "kind": "artifact",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/artifact-lab.stories.tsx",
      "storyId": "compositions-artifact-lab--studio",
      "channelIds": [
        "channel.campaign"
      ],
      "requiredTokenSets": [
        "color",
        "semantic.color",
        "component",
        "typography",
        "motion"
      ],
      "requiredComponentIds": [
        "component.badge",
        "component.button",
        "component.logo-mark"
      ],
      "requiredAssetIds": [
        "asset.logo.mark"
      ],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "brand-signal",
          "role": "signature",
          "required": true,
          "instruction": "Use the diagonal signal-cut geometry once as the recognisable gesture."
        },
        {
          "id": "claim-layer",
          "role": "content",
          "required": true,
          "instruction": "Render the approved claim and offer terms deterministically."
        },
        {
          "id": "format-safe-zone",
          "role": "constraint",
          "required": true,
          "instruction": "Preserve hierarchy across 1:1, 4:5, 9:16, and 16:9 adaptations."
        }
      ],
      "agent": {
        "requiredInputs": [
          "campaignObjective",
          "approvedClaim",
          "format"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions",
          "format",
          "claimApproval",
          "adaptation"
        ],
        "humanReviewTriggers": [
          "Any claim or offer",
          "Any generative background",
          "Any logo or legal-text placement"
        ]
      },
      "usage": {
        "do": [
          "Keep claims, CTAs, logos, and legal text deterministic.",
          "Record the source and review state of generated imagery."
        ],
        "dont": [
          "Bake legal or brand-critical content into generated pixels.",
          "Repeat the signature geometry as decoration."
        ]
      }
    },
    {
      "id": "pattern.presentation.decision-slide",
      "name": "Decision slide",
      "description": "A widescreen executive slide that frames one decision with evidence, consequence, and source provenance.",
      "kind": "artifact",
      "status": "beta",
      "introducedIn": "0.3.0",
      "source": "src/design-system/stories/artifact-lab.stories.tsx",
      "storyId": "compositions-artifact-lab--studio",
      "channelIds": [
        "channel.presentation"
      ],
      "requiredTokenSets": [
        "semantic.color",
        "component",
        "typography",
        "radius"
      ],
      "requiredComponentIds": [
        "component.badge",
        "component.eyebrow",
        "component.surface"
      ],
      "requiredAssetIds": [
        "asset.logo.mark"
      ],
      "requiredRuleIds": [
        "rule.no-raw-color",
        "rule.contrast-aa",
        "rule.agent-traceability"
      ],
      "slots": [
        {
          "id": "decision",
          "role": "content",
          "required": true,
          "instruction": "State the single decision or argument in the slide title."
        },
        {
          "id": "evidence",
          "role": "evidence",
          "required": true,
          "instruction": "Use one primary evidence unit with an editable source."
        },
        {
          "id": "consequence",
          "role": "content",
          "required": true,
          "instruction": "Make the implication or requested action explicit."
        }
      ],
      "agent": {
        "requiredInputs": [
          "audience",
          "decision",
          "evidence",
          "source"
        ],
        "requiredTraceFields": [
          "designSystemVersion",
          "brandVersionId",
          "channel",
          "theme",
          "policyStatus",
          "tokens",
          "components",
          "assets",
          "patterns",
          "unresolvedDecisions",
          "canvas",
          "decision",
          "evidenceSource"
        ],
        "humanReviewTriggers": [
          "External data or claim",
          "Non-editable brand layer",
          "Type below minimum size"
        ]
      },
      "usage": {
        "do": [
          "Keep each slide focused on one decision.",
          "Keep text, evidence, logo, and source editable."
        ],
        "dont": [
          "Shrink dense documents onto a slide.",
          "Use untraceable evidence."
        ]
      }
    }
  ]
} as const;

export const channelCatalog = {
  "$schema": "/design-system/schemas/channel-catalog.schema.json",
  "schemaVersion": "1.0.0",
  "version": "0.3.0",
  "brandVersionId": "wonka-brand-v0.1.0",
  "channels": [
    {
      "$schema": "/design-system/schemas/channel.schema.json",
      "schemaVersion": "1.0.0",
      "id": "channel.product",
      "name": "Product",
      "description": "Task-oriented interfaces for Wonka products, internal tools, and agent workspaces.",
      "aliases": [
        "product",
        "app"
      ],
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "allowedThemes": [
        "light",
        "dark"
      ],
      "distribution": "external",
      "tokenSets": [
        "color",
        "semantic.color",
        "radius",
        "shadow",
        "typography",
        "motion",
        "component"
      ],
      "compatibleAssetIds": [
        "asset.logo.wordmark",
        "asset.logo.mark",
        "asset.font.inter-display",
        "asset.font.gt-sectra"
      ],
      "constraints": [
        {
          "id": "responsive",
          "value": "Support 320px through large desktop layouts without horizontal scrolling.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "accessibility",
          "value": "Text and visible focus indicators meet their WCAG 2.2 AA contrast thresholds.",
          "level": "required",
          "enforcement": "automated",
          "ruleId": "rule.contrast-aa"
        },
        {
          "id": "keyboard",
          "value": "Every interactive flow remains operable with a keyboard and retains visible focus.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "reduced-motion",
          "value": "Non-essential motion respects the user’s reduced-motion preference.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "interface-states",
          "value": "Interactive elements define loading, empty, error, disabled, focus, and success states where applicable.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "distribution",
          "value": "Treat Product as externally distributed by default. Internal-only surfaces require a dedicated channel before verified_internal assets can be selected.",
          "level": "required",
          "enforcement": "contract"
        }
      ],
      "exportTargets": [
        "react",
        "css",
        "html"
      ],
      "provenance": {
        "source": "Wonka product primitives",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      },
      "compatibleComponentIds": [
        "component.button",
        "component.badge",
        "component.surface",
        "component.eyebrow",
        "component.logo",
        "component.logo-mark",
        "component.product.product-theme",
        "component.product.action-button",
        "component.product.icon",
        "component.product.identity-avatar",
        "component.product.panel",
        "component.product.field",
        "component.product.toggle",
        "component.product.tag",
        "component.product.search-input",
        "component.product.state-notice",
        "component.product.product-dialog",
        "component.product.motion-stage",
        "component.product.connector-card",
        "component.product.connector-configuration",
        "component.product.connector-authorization",
        "component.product.connector-catalog",
        "component.product.template-card",
        "component.product.template-gallery",
        "component.product.template-detail",
        "component.product.builder-section",
        "component.product.agent-builder-header",
        "component.product.model-picker",
        "component.product.skill-picker",
        "component.product.tool-picker",
        "component.product.knowledge-files",
        "component.product.schedule-editor",
        "component.product.version-history",
        "component.product.agent-builder",
        "component.product.product-shell",
        "component.product.product-agent-card",
        "component.product.agent-library",
        "component.product.chat-composer",
        "component.product.execution-trace",
        "component.product.source-citation",
        "component.product.chat-message",
        "component.product.deliverable-preview",
        "component.product.sharing-panel",
        "component.product.settings-navigation",
        "component.product.members-table",
        "component.product.resource-table",
        "component.product.usage-overview",
        "component.product.settings-panel",
        "component.product.settings-workspace",
        "component.product.skill-card",
        "component.product.skill-library",
        "component.product.skill-detail",
        "component.product.onboarding-checklist",
        "component.product.approval-card",
        "component.product.workflow-runs",
        "component.product.prompt-library",
        "component.product.notification-toast"
      ]
    },
    {
      "$schema": "/design-system/schemas/channel.schema.json",
      "schemaVersion": "1.0.0",
      "id": "channel.website",
      "name": "Website",
      "description": "Editorial and conversion-oriented browser experiences built from Wonka’s shared foundations.",
      "aliases": [
        "website",
        "web"
      ],
      "lifecycle": "review_required",
      "stability": "beta",
      "introducedIn": "0.2.0",
      "allowedThemes": [
        "light",
        "dark"
      ],
      "distribution": "external",
      "tokenSets": [
        "color",
        "semantic.color",
        "radius",
        "shadow",
        "typography",
        "motion",
        "component"
      ],
      "compatibleAssetIds": [
        "asset.logo.wordmark",
        "asset.logo.mark",
        "asset.font.inter-display",
        "asset.font.gt-sectra"
      ],
      "constraints": [
        {
          "id": "responsive",
          "value": "Support 320px through large desktop layouts without horizontal scrolling.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "accessibility",
          "value": "Text and visible focus indicators meet their WCAG 2.2 AA contrast thresholds.",
          "level": "required",
          "enforcement": "automated",
          "ruleId": "rule.contrast-aa"
        },
        {
          "id": "keyboard",
          "value": "Every interactive flow remains operable with a keyboard and retains visible focus.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "reduced-motion",
          "value": "Non-essential motion respects the user’s reduced-motion preference.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "explicit-actions",
          "value": "Use real product language and explicit action labels.",
          "level": "required",
          "enforcement": "human_review"
        }
      ],
      "exportTargets": [
        "react",
        "css",
        "html",
        "png",
        "pdf"
      ],
      "provenance": {
        "source": "Wonka website primitives",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      },
      "compatibleComponentIds": [
        "component.button",
        "component.badge",
        "component.section",
        "component.surface",
        "component.eyebrow",
        "component.logo",
        "component.logo-mark"
      ]
    },
    {
      "$schema": "/design-system/schemas/channel.schema.json",
      "schemaVersion": "1.0.0",
      "id": "channel.campaign",
      "name": "Campaign",
      "description": "Paid, organic, event, and launch creative that remains recognisably Wonka at multiple aspect ratios.",
      "aliases": [
        "campaign"
      ],
      "lifecycle": "review_required",
      "stability": "experimental",
      "introducedIn": "0.2.0",
      "allowedThemes": [
        "light",
        "dark"
      ],
      "distribution": "external",
      "tokenSets": [
        "color",
        "semantic.color",
        "typography",
        "motion",
        "radius",
        "component"
      ],
      "compatibleAssetIds": [
        "asset.logo.wordmark",
        "asset.logo.mark",
        "asset.font.inter-display",
        "asset.font.gt-sectra"
      ],
      "constraints": [
        {
          "id": "deterministic-brand-layer",
          "value": "Render logos, copy, prices, CTAs, and legal text deterministically after any generative background step.",
          "level": "required",
          "enforcement": "human_review"
        },
        {
          "id": "claim-approval",
          "value": "Claims and offer terms require explicit human approval before publication.",
          "level": "required",
          "enforcement": "human_review"
        },
        {
          "id": "adaptation",
          "value": "Preserve hierarchy and safe zones when adapting 1:1, 4:5, 9:16, and 16:9 formats.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "signature",
          "value": "Use the signal-cut geometry once as the expressive brand gesture; avoid generic decorative gradients.",
          "level": "recommended",
          "enforcement": "human_review"
        }
      ],
      "exportTargets": [
        "png",
        "jpg",
        "mp4",
        "html"
      ],
      "provenance": {
        "source": "Wonka Brand DNA and AI Creative Studio contracts",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      },
      "compatibleComponentIds": [
        "component.button",
        "component.badge",
        "component.section",
        "component.surface",
        "component.eyebrow",
        "component.logo",
        "component.logo-mark",
        "component.product.product-theme",
        "component.product.action-button",
        "component.product.icon",
        "component.product.identity-avatar",
        "component.product.panel",
        "component.product.field",
        "component.product.toggle",
        "component.product.tag",
        "component.product.search-input",
        "component.product.state-notice",
        "component.product.product-dialog",
        "component.product.motion-stage",
        "component.product.connector-card",
        "component.product.connector-configuration",
        "component.product.connector-authorization",
        "component.product.connector-catalog",
        "component.product.template-card",
        "component.product.template-gallery",
        "component.product.template-detail",
        "component.product.builder-section",
        "component.product.agent-builder-header",
        "component.product.model-picker",
        "component.product.skill-picker",
        "component.product.tool-picker",
        "component.product.knowledge-files",
        "component.product.schedule-editor",
        "component.product.version-history",
        "component.product.agent-builder",
        "component.product.product-shell",
        "component.product.product-agent-card",
        "component.product.agent-library",
        "component.product.chat-composer",
        "component.product.execution-trace",
        "component.product.source-citation",
        "component.product.chat-message",
        "component.product.deliverable-preview",
        "component.product.sharing-panel",
        "component.product.settings-navigation",
        "component.product.members-table",
        "component.product.resource-table",
        "component.product.usage-overview",
        "component.product.settings-panel",
        "component.product.settings-workspace",
        "component.product.skill-card",
        "component.product.skill-library",
        "component.product.skill-detail",
        "component.product.onboarding-checklist",
        "component.product.approval-card",
        "component.product.workflow-runs",
        "component.product.prompt-library",
        "component.product.notification-toast"
      ]
    },
    {
      "$schema": "/design-system/schemas/channel.schema.json",
      "schemaVersion": "1.0.0",
      "id": "channel.presentation",
      "name": "Presentation",
      "description": "Executive, sales, workshop, and product decks that share the same tokens and branded hierarchy.",
      "aliases": [
        "presentation"
      ],
      "lifecycle": "review_required",
      "stability": "experimental",
      "introducedIn": "0.2.0",
      "allowedThemes": [
        "light",
        "dark"
      ],
      "distribution": "external",
      "tokenSets": [
        "color",
        "semantic.color",
        "typography",
        "radius",
        "component"
      ],
      "compatibleAssetIds": [
        "asset.logo.wordmark",
        "asset.logo.mark",
        "asset.font.inter-display",
        "asset.font.gt-sectra"
      ],
      "constraints": [
        {
          "id": "canvas",
          "value": "Use a 16:9 widescreen canvas unless the destination requires another format.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "minimum-type",
          "value": "Use at least 18pt body text and 12pt source or footnote text.",
          "level": "required",
          "enforcement": "contract"
        },
        {
          "id": "one-idea",
          "value": "Each slide carries one decision, argument, or evidence unit.",
          "level": "recommended",
          "enforcement": "human_review"
        },
        {
          "id": "editable",
          "value": "Keep text, charts, logos, and legal content editable and deterministic.",
          "level": "required",
          "enforcement": "human_review"
        }
      ],
      "exportTargets": [
        "pptx",
        "pdf",
        "png"
      ],
      "provenance": {
        "source": "Wonka deck and website shared-system goal",
        "owner": "Wonka Design System",
        "approvedBy": null,
        "approvedAt": null
      },
      "compatibleComponentIds": [
        "component.badge",
        "component.surface",
        "component.eyebrow",
        "component.logo",
        "component.logo-mark"
      ]
    }
  ]
} as const;

export type RuleId = (typeof ruleCatalog.rules)[number]["id"];
export type PatternId = (typeof patternCatalog.patterns)[number]["id"];
export type ChannelId = (typeof channelCatalog.channels)[number]["id"];
export type ChannelAlias = (typeof channelCatalog.channels)[number]["aliases"][number];
